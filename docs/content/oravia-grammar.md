...
<p>No grammatical gender (except optional suffixes)</p>
...
<h4>2.3 Stress</h4>
<p>Stress always falls on the penultimate syllable (second to last). The exception is when you have a suffix, which is stressed.</p>
<pre>coupa    → COU-pa
eledora  → e-le-DO-ra
lidastor → li-DAS-tor
anparu → an-PA-ru
anparum (anparu+um) → an-pa-RUM</pre>
...
<tr><td>de</td><td>of / from</td><td>de nime faibor = of my spouse</td></tr>
...
<pre>❌  [nim] en [i copei] nime yamirli bo     (en split from its block)
❌  [nim] [i copei] en nime bo yamirli     (yamirli after what it modifies)</pre>
<p>Within a block, modifiers always come before what they modify. A long noun phrase like de yamirli heivio haijor (of the old magic witch) reads left to right, with each word narrowing the concept of what follows.</p>
<h4>3.3 Dropping Markers</h4>
<p>Markers can be omitted when meaning is clear from context. The a (subject) marker drops most easily, especially with pronouns. The e marker drops when the object is unambiguous. The u marker is rarely dropped.</p>
<pre>Formal:          a hay i mo e mocen
Casual:          hay i mo e mocen         (a dropped)
Very casual:     hay i mo mocen           (a and e dropped)</pre>
</div>
</details>
...
<p>hay is fully gender-neutral. It covers he, she, it, and any singular third person.</p>
...
<pre>varodu toulum      → head hurts (not: my head)
farejor i anifi     → mother is coming (not: my mother)
i siur e yesrel      → remove the jacket (not: your jacket)</pre>
<p>Many sentences don&#x27;t have a subject. An affirmative sentence without a subject defaults to &quot;I&quot;, and a question defaults to &quot;you&quot;, like this:</p>
<pre>i ilianum = [V] know-not = I don&#x27;t know 
ce i anye? = what [V] do? = what are you doing?</pre>
<p>When you connect clauses, if the subject changes from one clause to the next, you must introduce the new subject. This is true even if you would normally not use &quot;I&quot; or &quot;you&quot; if the clauses were their own sentences. This is to prevent ambiguity:</p>
<pre>haya i ilian ca i anye = they know what [they] are doing
...
hay i anifar noli          → she just left
...
haya i damai de litam cali nim i anifar
...
<details class="gr-sec">
<summary>12. The hai and hue Suffixes</summary>
<div class="gr-content">
<h4>12.1 hai — Agent and Role</h4>
<p>hai creates agent nouns: &quot;the one who does X&quot; or &quot;the one whose role is X&quot;:</p>
<pre>anye hai        → maker, creator
boemo hai       → cook
be hai          → traveler
heivio hai      → witch / wizard</pre>
<p><em>To remember it: </em>hai contains a (actor) + i (verb), the actor-verb combination.</p>
...
<details class="gr-sec">
<summary>14. Negation Scope</summary>
<div class="gr-content">
...
<tr><td>cemir</td><td>ce + mir (many)</td><td>how many?</td></tr>
...
cemir falen i dami?       → How many children are there?
...
caora nim i anifar, i doum i anivari  → because I left, I couldn&#x27;t return</pre>
...
nim i anvu cane run                  → I go as much as you (to the same degree)</pre>
<p>How much as in price follows the same logic: ce + gaolei (what + price, value), and you can create new question words by making other combinations.</p>
</div>
</details>
...
<p>The marker immediately before ce signals the role of the unknown. Dedicated question words like cedom and cei are shortcuts for the most common cases (en ce for where, a ce + person for who), but the marker-plus-ce construction is available for any role.</p>
...
<p>Ca introduces an embedded content clause: a fact, a claim, or a proposition. If we don&#x27;t know whether something is true, we use &quot;ia dou&quot; (yes or...), which means &quot;whether&quot;.</p>
<h5>Examples</h5>
<table>
...
<p><em>i ilianum ca </em><em>hay</em><em> i dairan</em><em>. (V know-not connector he V like)</em></p>
...
<tr><td>ilwol</td><td>thing</td><td>i ilianum ilwol ca hay i dairan →  I don&#x27;t know the thing he likes</td></tr>
<tr><td>elireva</td><td>truth, fact</td><td>i ilianum elireva ca hay i dairan →  I don&#x27;t know the fact  that he likes it</td></tr>
</table>
...
<p>su is versatile: it connects nouns, verbs, and full sentences:</p>
<pre>i mo e mocen su e moaria    → I eat chocolate and apple
i mo su i mouje             → I eat and drink
...
<div class="gr-content">
<h4>19.1 Cardinal Numbers 1–10</h4>
<table>
<tr><th>1</th><th>2</th><th>3</th><th>4</th><th>5</th><th>6</th><th>7</th><th>8</th><th>9</th><th>10</th></tr>
<tr><td>tam</td><td>tor</td><td>par</td><td>balem</td><td>alui</td><td>teva</td><td>peri</td><td>auta</td><td>tenda</td><td>das</td></tr>
</table>
<h4>19.2 Numbers Beyond 10</h4>
<p>Larger unit first:</p>
<pre>das tam = 11    das par = 13    tor das = 20
par das = 30    par das balem = 34</pre>
<h4>19.3 Ordinals</h4>
...
liperiar = last week   liperaris = next week</pre>
<h4>19.5 Large Numbers — nen / ponen</h4>
<p>ponen (power), short form nen, base 10:</p>
...
<details class="gr-sec">
<summary>20. Fractional Numbers</summary>
<div class="gr-content">
...
hay i none e tam de balem de nen tor  → she has a quarter of 100
a tam de tor de lidastor i dami     → half a year remains (ligo can also be used, meaning &quot;half&quot;)</pre>
<p>For common fractions in everyday speech, the ne gradation system often serves just as well: nealui = 5/10 = half, neteva = 6/10 etc. Use the de construction when the exact fraction matters.</p>
</div>
</details>
...
<h4>22.3 Imported Words and Scientific Names</h4>
...
a yamirli heivio hai    → an old witch</pre>
<h4>23.4 Explicit Markers</h4>
<p>For absolute clarity, use the explicit role markers:</p>
...
<p>These explicit markers are optional in most contexts but resolve ambiguity in complex sentences.</p>
</div>
</details>
...
ga nime fasujal, a nim a yamirli      → I am older than my brother
a bo a yaltan ga rune            → my house is bigger than yours</pre>
<h4>24.2 Equality — gaomem</h4>
<p>gaomem means &quot;the same.&quot; With ga it expresses &quot;as [adjective] as&quot;:</p>
<pre>ga run, a nim a gaomem yalen          → I am as tall as you
a haye dairan a gaomem de nime        → her preference is the same as mine</pre>
...
a fatore a anodu yunro de fatore  → my teacher is the smartest among teachers</pre>
</div>
</details>
...
<p>With lirul (habit) — frequency:</p>
...
<p>With a context word — precise descriptions:</p>
<pre>nebalem antori = 4/10 open = ajar
...
<p>Oravia has no plural morphology. The bare noun covers both singular and plural; context usually makes the number clear:</p>
<pre>miau i bonfene         → a cat is sleeping / cats are sleeping</pre>
<p>When explicit plurality is needed, use mir (many) or a number directly before the noun:</p>
<pre>mir miau               → many cats
tor miau               → two cats
...
<p>negafei is from the NE cluster (quantifiers) + gafi (less).</p>
...
<pre>nime bo         → my house
o nime bo       → my OWN house / specifically my house
de elihei       → of self (if additional clarity is needed)</pre>
<p>The o marker already in the language carries the meaning; no new vocabulary needed.</p>
</div>
</details>
</div>
</details>
...
<div class="gr-content">
<h4>29.1 o e — Emphasis on object (event passive)</h4>
<p>The most common passive uses the o marker before the e-marked object. The subject recedes and the object becomes the focus:</p>
...
<div class="gr-content">
<p>bi introduces the speaker’s emotional reaction or editorial commentary: not neutral content, but how the speaker feels about what is being said. bi phrases can be inserted anywhere and can stand alone:</p>
<pre>nim i faigel bi oipoh!     → I&#x27;m getting married — how exciting!
bi oila a nim en bo        → I&#x27;m so happy to be home</pre>
<p>Alone, bi phrases work as pure exclamation:</p>
...
<details class="gr-sec">
<summary>31. Sentence-Final and Clause-Final Particles</summary>
<div class="gr-content">
<p>Many languages use small particles at the end of sentences to add emotional tone, epistemic stance, or pragmatic coloring. Oravia handles most of this through a small set of elements that can appear in sentence-final or clause-final position.</p>
<h4>31.1 bi — Emotional / Editorial Comment</h4>
<p>bi is the primary one. It can appear anywhere but naturally gravitates toward sentence-final or pre-comment positions. We covered this under §31</p>
<h4>31.2 ilie — Epistemic Hedging</h4>
<p>ilie (maybe/likelihood) at the end of a sentence softens it into a possibility or expresses the speaker&#x27;s uncertainty:</p>
...
<p>The system is deliberately minimal; these three elements combine to cover a wide range of pragmatic coloring.</p>
</div>
</details>
...
<pre>iliciu i ilian canon, i anye → if I knew how, I would do it
iliciu i moarum, toumo → if I hadn&#x27;t eaten, I would have been hungry</pre>
<h4>36.2 daehun — Factual If-Then</h4>
<p>Use daehun for logical truths and real-world cause-and-effect:</p>
<pre>daehun nim i apanou e moaria, a moaria i apanou
if I drop the apple, it falls</pre>
<p>Notice that by using iliciu and daehun, we avoid many complicated verb forms in a lot of languages including English.</p>
</div>
</details>
...
<pre>neron hay i &#x27;asufu, neron be hai i vanta e jovabo en hay
...
<p>For the reverse (&quot;the more X, the less Y&quot;), combine neron with gafi or a negated form:</p>
<pre>neron nim i bonfene, gafi nim i apavu
...
<pre>i jadun             → I eat with a fork
     i beivu             → I go by car</pre>
...
<pre>a nim a tohpu caora siur de haye   → I am sad because of her disappearance
...
<pre>i boemo u lor run               → I cook for your benefit (as a kindness)
...
<p>This maps some natural languages, for example Japanese ageru and Portuguese por as distinct from para. u lor has the flavor of a deeper gift or act of service.</p>
</div>
</details>
...
<p>no also serves as a nominal complement — it introduces what something is called, considered, or perceived as. </p>
...
i iliro ciude run no elonemi hai   → I think about you as a savior</pre>
<p>The pattern generally is [verb] e [object] no [characterization], where the no phrase tells you what the object is being called, judged, or perceived as. Compare with the copula for direct identity:</p>
...
<div class="gr-content">
<p>nomie means &quot;to move/seem/appear&quot; in a general sense. Combined with no (like/as), it covers the English &quot;it seems,&quot; &quot;it appears,&quot; &quot;it looks like,&quot; etc.:</p>
...
<p>The sensory word specifies the modality; no introduces the comparison. Without a sensory word, nomie no is the general &quot;seems/appears like&quot; applicable to any perception or impression.</p>
</div>
</details>
...
<div class="gr-content">
<p>Causative constructions use verb stacking with ponen (power, to impose). The one being made to do something takes the e (object) marker as the direct object of pohnen. The caused action follows with i:</p>
<pre>i ponen ca e [causee]  i [caused action]
 
i ponen ca e hay i mo
I made that him eat
(I imposed on him: eat)
 
farejor i ponen ca e nim i bonfene
my mother made me lie down
 
haya i ponen ca e falen i ilaluan
they made the child speak</pre>
<p>&quot;Ca&quot; may be omitted if it&#x27;s clear.</p>
<p>The strength of imposition scales with context. ponen implies genuine forcing or imposition. </p>
<p>For a softer causative, we use &quot;antai&quot;, to guide:</p>
<pre>i antai ca e [causee]  i [caused action]
...
<p>When we are talking about emotions, states, or general things that do not require that the affected person perform an action, we use a different construction. In this case, an omitted subject means a vague cause, and you use the e marker for the person affected:</p>
<pre>i [state] e [person affected]
 
i tohpu e nim
this makes me sad
 
elenom i toulum e falen
the journey tires the child

a haye ilaluan i oipoh e nim 
her words excite me</pre>
</div>
</details>
...
<div class="gr-content">
<p>domvio (dom = place + davio = change) means &quot;instead,&quot; as in: in the place of, as a substitute for. It works as a relational word introducing the thing being replaced:</p>
...
<p>Any word becomes a verb when placed after i. For many word types, the meaning follows the default <em>to use it in its primary function</em>: </p>
<table>
...
<tr><td>Vehicle</td><td>to travel by</td><td>i beivu = to go by car</td></tr>
</table>
...
<p>English uses &quot;how&quot; for both manner and degree. Oravia keeps these separate. Cenon asks in what way something is done; cene asks to what extent. &quot;How sad are you?&quot; is a question about degree, not about method.</p>
<pre>How sad are you?
...
<p>. Daehun is for real-world cause and effect. </p>
...
<p>Using daehun for all three flattens a distinction the grammar is built to preserve, and confuses listeners into thinking an &quot;if-then&quot; complement is coming.</p>
...
<p>Compounds are two or more words that together name a single concept rather than describing one thing with an adjective. The first word takes the suffix -a to signal that it is part of a compound rather than a free modifier (see §22.1)</p>
<p>Hyphenated forms join two words for fine-grained or expressive meaning, creating a reading that neither word carries alone. Ilofun-vardei (hesitant-look, to peek with doubt) and toului-asfe (tired-stop, to give up from exhaustion) are examples. The hyphen signals conceptual fusion rather than a modification relationship (see §)</p>
<p>Imported words use an apostrophe prefix to signal that the word does not come from the core vocabulary: &#x27;Italia, &#x27;Nihongo, scientific species names. These never enter the permanent vocabulary. The apostrophe tells the reader this is not a core Oravia word (see §22.2)</p>
...
<p>Third, combine. Cluster domain plus root meaning gives you the approximate reading. This will not always be exact, but it puts you in the right territory. </p>
<p>Two examples:</p>
<pre>vardei
  VA = body parts
  VAR = face (subcluster)
...
<p>When learning or encountering words, keeping syllable-meaning associations in mind will do more than memorizing individual words. </p>
</div>
</details>
...
<tr><td>AN</td><td>action / movement</td><td>ANE static · ANI toward · ANO away · APA physical · AS involuntary body · ASE affectionate contact · ASI orifice · ASU involuntary vocal</td></tr>
...
<tr><td>-e</td><td>possessive suffix on pronouns</td></tr>
...
<tr><td>-a</td><td>compound modifier suffix</td></tr>
<tr><td>hai / hue</td><td>agent, role / recipient, resultant state</td></tr>
...
<tr><td>cemir / cenon / cene</td><td>question words: how many / how (manner) / how (degree)</td></tr>
...
<tr><td>iliciu / daehun</td><td>counterfactual / factual conditional</td></tr>
<tr><td>lirul</td><td>habit</td></tr>
<tr><td>notam / notor</td><td>first event / second event</td></tr>
<tr><td>yadetu / tonan / lar / lis</td><td>finally / suddenly / once (past) / sometime in the future</td></tr>
