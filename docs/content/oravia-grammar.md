...
Nearly all auxlangs are essentially simplified Romance-Germanic: even when they broaden the vocabulary, the structure and logic are largely untouched, just replacing the words (easy way to see this: check the interlinear gloss). Oravia avoids Eurocentric bias by having a **different grammatical system inspired by languages around the world**. For example, the noun cluster system draws from Bantu noun classes, the marker and zero-pronoun system from Japanese, and the verb aspect system from Yucatec Maya. In the quest of building Oravia, I asked a simple question: *how can something be expressed in the most elegant way, with the least load?*
...
[Open PDF](oravia-grammar.pdf)

<iframe src="../oravia-grammar.pdf" width="100%" height="800px"></iframe>
