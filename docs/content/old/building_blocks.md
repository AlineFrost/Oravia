...
(async function() {
  ...
  function render() {
    ...
    let filtered = allData.filter(r => {
      ...
      if (q) {
        const hay = (r.sound+' '+r.meaning+' '+r.oravia.join(' ')+' '+r.english.join(' ')+' '+(r.natural_language||'')).toLowerCase();
  ...
  // Read ?q= URL parameter and pre-fill search
  const urlParams = new URLSearchParams(window.location.search);
  const qParam = urlParams.get('q');
  if (qParam) {
    document.getElementById('bb-search').value = qParam;
  }
