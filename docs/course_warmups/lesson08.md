...
=== "Grammar"
    ...
    For **possessives** we just use the pronoun before the noun, like any other modifier:  
    
    ```
    nim mocen = my chocolate  
    haya bo = their house  
    ```
    
    Notice that when it's obvious, you don't have to specify at all:  
    ```
    faibor = my spouse
    nim i anvu en bo = I am going to my house
    ```
    
    This is for pronouns (nim, run, hay, nima, runa, haya). For all other words, we use **de**:  
    
    ```
    mocen de farejor = mom's chocolate
    bo de Demi = Demi's house
    ```
